      ...
      <div className='flex flex-col gap-24'>
        <div className='flex flex-col md:flex-row justify-between items-end gap-10'>
          <div className='max-w-4xl flex flex-col gap-8'>
            ...
            {subtitle && (
              <p className='text-lg md:text-xl text-dark/70 font-poppins max-w-2xl'>
                {subtitle}
              </p>
            )}
          </div>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-dark/10 border border-dark/10'>
          {items.map((item, idx) => (
            <motion.div 
              ...
              viewport={{ once: true }}
              className='group bg-white p-12 hover:bg-accent/5 transition-all duration-500 flex flex-col gap-16 relative overflow-hidden'
            >
              {/* Decorative Tech Icon */}
              <div className='absolute -top-10 -right-10 w-40 h-40 bg-accent/10 rounded-full blur-3xl group-hover:bg-accent/20 transition-colors' />

              <div className='flex flex-col gap-6 relative z-10'>
                <h3 className='text-h3 font-bebas text-dark uppercase leading-none group-hover:text-accent transition-colors duration-500'>
                  {item.title}
                </h3>
